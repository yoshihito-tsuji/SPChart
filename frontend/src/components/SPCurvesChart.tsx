...
import { getChartPNG, downloadImage } from '../utils/export';

interface SPCurvesChartProps {
  result: SPTableResult;
}

export interface SPCurvesChartHandle {
  downloadPNG: () => void;
  getEChartsInstance: () => EChartsInstance | null;
}

export const SPCurvesChart = forwardRef<SPCurvesChartHandle, SPCurvesChartProps>(
  function SPCurvesChart({ result }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      downloadPNG: () => {
        ...
        const dataUrl = getChartPNG(instance);
        if (dataUrl) {
          const timestamp = new Date().toISOString().slice(0, 10);
          downloadImage(dataUrl, `sp-curves-${timestamp}.png`);
        }
      },
