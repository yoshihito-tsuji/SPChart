...
Phase 1の実装が完了しました（コミット `8221d5c`）。

#### 実装したファイル群

**コア計算モジュール (`frontend/src/core/sp/`)**
| ファイル | 内容 |
|----------|------|
| `sortTable.ts` | 安定ソートによる並べ替え（originalIndex保持） |
| `computeCurves.ts` | S曲線・P曲線の算出（0-1正規化） |
| `computeCautionIndices.ts` | 注意係数CS/CPの計算（分母0→null） |
| `computeDisparityCoefficient.ts` | 差異係数D*の計算 |
| `index.ts` | analyzeSPTable統合関数、統計サマリ計算 |

**型定義 (`frontend/src/types/sp.ts`)**
- RawTestData, SPTableResult, SortedStudent, SortedProblem
- CautionLevel, getCautionLevel, CAUTION_THRESHOLDS

**UIコンポーネント (`frontend/src/components/`)**
| コンポーネント | 内容 |
|----------------|------|
| `FileUploader.tsx` | CSV読込 |
| `SampleDataSelector.tsx` | サンプルデータ選択ボタン |
| `SPTableGrid.tsx` | S-P表グリッド表示（2段階ハイライト対応） |
| `SPCurvesChart.tsx` | EChartsによるS/P曲線描画 |
| `StatsSummary.tsx` | 統計サマリ・凡例表示 |
| `LoadingSkeleton.tsx` | ローディング状態表示 |

**サンプルデータ (`frontend/src/data/sampleData.ts`)**
- 5×5（最小テスト、完全Guttmanパターン）
- 30×20（標準クラス規模）
- 300×60（学年規模）
...
| サンプル | D* | 要注意生徒 | 要注意問題 | 計算時間 |
|----------|-----|-----------|-----------|---------|
| 5×5 | **0.000** | 0名 | 0問 | <1ms |
| 30×20 | 0.918 | 23名 | 13問 | <1ms |
| 300×60 | 0.949 | 282名 | 60問 | 2.16ms |

- 5×5で D*=0 を確認（完全Guttmanパターン）✓
- 300×60でも2ms台で計算完了（パフォーマンス良好）✓

#### 入力方針（確定）

- **中規模（〜300名×60問）**: フォーム入力を正式サポート
- **大規模**: CSVアップロード推奨
- **フォーム仕様**: 値は0/1のみ、1行目=問題ID、1列目=生徒ID
- CSV保存機能は次フェーズで検討
